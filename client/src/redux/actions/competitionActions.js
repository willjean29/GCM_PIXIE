...
export const activarConcursoAction = (concurso) => {
  return async (dispatch) => {
    ...
    try {
      const response = await clienteAxios.post(
        `/competition/active/${concurso._id}`
      );
      const data = response.data;
      Notification(data.ok, data.msg);
...
export const eliminarConcursoAction = (concurso) => {
  return async (dispatch) => {
    dispatch(eliminarConcurso());
    tokenAuthAdmin();
    try {
      const response = await clienteAxios.delete(
        `/competition/${concurso._id}`
      );
      const data = response.data;
      Notification(data.ok, data.msg);
      dispatch(eliminarConcursoOk());
    } catch (error) {
      console.log(error.response);
      const msg = error.response.data
        ? error.response.data.err.msg
        : "Hubo un error";
      Notification(error.response.data.ok, msg);
      dispatch(eliminarConcursoError());
    }
  };
};
...
const eliminarConcursoOk = () => ({
  type: DELETE_COMPETITION_OK,
});
const eliminarConcursoError = () => ({
